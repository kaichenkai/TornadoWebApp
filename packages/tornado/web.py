...
class RequestHandler(object):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        # templates namespace.  Historically only `modules` was available
        # but could be clobbered by user additions to the namespace.
        # The templates {% module %} directive looks in `_tt_modules` to avoid
        # possible conflicts.
    ...
    """Hook for subclass env_init. Called for each request.
    ...
    def prepare(self) -> Optional[Awaitable[None]]:
        ...
        Override this method to perform common env_init regardless
    ...
    def render(self, template_name: str, **kwargs: Any) -> "Future[None]":
        """Renders the templates with the given arguments as the response.
    ...
    def render_string(self, template_name: str, **kwargs: Any) -> bytes:
        """Generate the given templates with the given arguments.

        We return the generated byte string (in utf8). To generate and
        write a templates as a response, use render() above.
        """
    ...
    def get_template_namespace(self) -> Dict[str, Any]:
        """Returns a dictionary to be used as the default templates namespace.

        May be overridden by subclasses to add or modify values.

        The results of this method will be combined with additional
        defaults in the `tornado.templates` module and keyword arguments
    ...
    def create_template_loader(self, template_path: str) -> template.BaseLoader:
        """Returns a new templates loader for the given path.
    ...
    def get_template_path(self) -> Optional[str]:
        """Override to customize templates path for each handler.
    ...
    def xsrf_form_html(self) -> str:
        ...
        In a templates, this method should be called with ``{% module
        xsrf_form_html() %}``
    ...
    def static_url(self, path: str, include_host: bool = None, **kwargs: Any) -> str:
        ...
        This method requires you set the ``statics`` setting in your
        ...
        """
        self.require_setting("statics", "static_url")
...
class Application(ReversibleRouter):
    ...
    statics setting described below)::
    ...
    You can serve static files by sending the ``statics`` setting
    ...
    def __init__(
        ...
        **settings: Any
        ...
        if self.settings.get("statics"):
            path = self.settings["statics"]
...
class _HandlerDelegate(httputil.HTTPMessageDelegate):
    ...
    def execute(self) -> Optional[Awaitable[None]]:
        # If templates cache is disabled (usually in the debug mode),
...
class StaticFileHandler(RequestHandler):
    ...
    ``statics`` keyword argument to `Application`.  This handler
    ...
    To serve a file like ``statistics.html`` automatically when a directory is
    requested, set ``static_handler_args=dict(default_filename="statistics.html")``
    ...
    **Subclassing notes**
    ...
    def get_absolute_path(cls, root: str, path: str) -> str:
        ...
        (in most cases the ``statics`` `Application` setting).
    ...
    def parse_url_path(self, url_path: str) -> str:
        ...
        filesystem path relative to ``statics``.
    ...
    def get_version(cls, settings: Dict[str, Any], path: str) -> Optional[str]:
        ...
        """
        abs_path = cls.get_absolute_path(settings["statics"], path)
...
class UIModule(object):
    ...
    def render_string(self, path: str, **kwargs: Any) -> bytes:
        """Renders a templates and returns it as a string."""
...
class TemplateModule(UIModule):
    """UIModule that simply renders the given templates.

    {% module Template("foo.html") %} is similar to {% include "foo.html" %},
    but the module version gets its own namespace (with kwargs passed to
    Template()) instead of inheriting the outer templates's namespace.

    Templates rendered through this module also get access to UIModule's
    automatic javascript/css features.  Simply call set_resources
    inside the templates and give it keyword arguments corresponding to
    the methods on UIModule: {{ set_resources(js_files=static_url("my.js")) }}
    Note that these resources are output once per templates file, not once
    per instantiation of the templates, so they must not depend on
    any arguments to the templates.
    """
            ...
            if path not in self._resource_dict:
                ...
                if self._resource_dict[path] != kwargs:
                    raise ValueError(
                        ...
                        "resources for the same templates"
                    )
            return ""
