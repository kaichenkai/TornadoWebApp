...
"""A simple templates system that compiles templates to Python code.

Basic usage looks like::

    t = templates.Template("<html>{{ myvalue }}</html>")
    print(t.generate(myvalue="XXX"))

`Loader` is a class that loads templates from a root directory and caches
the compiled templates::

    loader = templates.Loader("/home/btaylor")
...
Unlike most other templates systems, we do not put any restrictions on the
...
functions in to your templates just like any other variable
(In a `.RequestHandler`, override `.RequestHandler.get_template_namespace`)::

   ### Python code
   def add(x, y):
      return x + y
   templates.execute(add=add)
...
Variable names beginning with ``_tt_`` are reserved by the templates
...
``{% apply *function* %}...{% end %}``
    Applies a function to the output of all templates code between ``apply``
    and ``end``::
...
``{% block *name* %}...{% end %}``
    ...
    Blocks in the parent templates will be replaced with the contents of
    the same-named block in a child templates.::
...
``{% comment ... %}``
    A comment which will be removed from the templates output.  Note that
    there is no ``{% end %}`` tag; the comment goes from the word ``comment``
    to the closing ``%}`` tag.

``{% extends *filename* %}``
    Inherit from another templates.  Templates that use ``extends`` should
    contain one or more ``block`` tags to replace content from the parent
    templates.  Anything in the child templates not contained in a ``block``
...
``{% include *filename* %}``
    Includes another templates file.  The included file can see all the local
    variables as if it were copied directly to the point of the ``include``
    directive (the ``{% autoescape %}`` directive is an exception).
    Alternately, ``{% module Template(filename, **kwargs) %}`` may be used
    to include another templates with an isolated namespace.
...
``{% module *expr* %}``
    ...
    when the templates system is used on its own in other contexts.
...
class Template(object):
    """A compiled templates.

    We compile into Python from the given template_string. You can generate
    the templates from variables with generate().
    """

    # note that the constructor's signature is not extracted with
    # autodoc because _UNSET looks like garbage.  When changing
    # this signature update website/sphinx/templates.rst too.
    def __init__(
        self,
        ...
        :arg str template_string: the contents of the templates file.
        :arg str name: the filename from which the templates was loaded
            (used for error message).
        :arg tornado.templates.BaseLoader loader: the `~tornado.templates.BaseLoader` responsible
            for this templates, used to resolve ``{% include %}`` and ``{% extend %}`` directives.
        :arg bool compress_whitespace: Deprecated since Tornado 4.3.
            Equivalent to ``whitespace="single"`` if true and
            ``whitespace="all"`` if false.
        :arg str autoescape: The name of a function in the templates
        ...
        try:
            ...
            # The dont_inherit flag prevents templates.py's future imports
    ...
    def generate(self, **kwargs: Any) -> bytes:
        """Generate this templates with the given arguments."""
        namespace = {
        ...
        # we've generated a new templates (mainly for this module's
    ...
    def _get_ancestors(self, loader: Optional["BaseLoader"]) -> List["_File"]:
        ...
        for chunk in self.file.body.chunks:
            if isinstance(chunk, _ExtendsBlock):
                if not loader:
                    raise ParseError(
                        "{% extends %} block found, but no " "templates loader"
                    )
...
class BaseLoader(object):
    """Base class for templates loaders.

    You must use a templates loader to use templates constructs like
    ...
    def __init__(
        ...
        """Construct a templates loader.

        :arg str autoescape: The name of a function in the templates
            namespace, such as "xhtml_escape", or ``None`` to disable
            autoescaping by default.
        :arg dict namespace: A dictionary to be added to the default templates
            namespace, or ``None``.
        ...
        # threads tried to compile the same templates simultaneously.
...
class Loader(BaseLoader):
    """A templates loader that loads from a single root directory.
    """
...
class DictLoader(BaseLoader):
    """A templates loader that loads from a dictionary."""
...
class ParseError(Exception):
    """Raised for templates syntax errors.
